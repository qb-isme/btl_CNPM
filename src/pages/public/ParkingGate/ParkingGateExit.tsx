...
export default function ParkingGateExit() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <Header status={systemStatus} gateName="Cổng X" operatorName="Tên NV Vận hành" gateType="exit" />
      ...
      <main className="px-6 pb-6">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-4">
          <button onClick={cycleExitStatus} className="rounded-lg bg-slate-700 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800">
            Trạng thái: {exitStatus}
          </button>
          <button onClick={toggleCustomerType} className="rounded-lg bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-700">
            Loại khách: {customerType === "visitor" ? "Vãng lai" : "Nội bộ"}
          </button>
          <Link to="/parking-gate" className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
            Xem luồng Xe Vào
          </Link>
        </div>
      </main>
