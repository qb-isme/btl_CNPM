...
export default function ParkingGate() {
  ...
  const handleCreateSession = () => {
    console.log("Creating session:", { plate: detectedPlate, cardId, customerType, fullName, employeeId })
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <Header status={systemStatus} gateName="Cổng X" operatorName="Tên NV Vận hành" gateType="entry" />
      <main className="px-6 pb-6">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          <div>
            <CameraFeed detectedPlate={detectedPlate} detectionStatus={detectionStatus} onManualEntry={handleManualEntry} isManualEntryActive={isManualEntry} />
          </div>
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-4">
          <button onClick={cycleStatus} className="rounded-lg bg-slate-700 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800">
            Trạng thái hệ thống: {systemStatus}
          </button>
          <button onClick={cycleError} className="rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700">
            Trạng thái lỗi: {formStatus}
          </button>
          <Link to="/parking-gate/xe-ra" className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
            Xem luồng Xe Ra
          </Link>
        </div>
      </main>
