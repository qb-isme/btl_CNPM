import { AlertCircle, AlertTriangle, CheckCircle, ChevronDown, User } from "lucide-react"
...
export function ExitCustomerInfo({
  ...
  const getCustomerTypeLabel = (type: CustomerType) => (type === "visitor" ? "Khách vãng lai" : "NỘI BỘ (SINH...")
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-2xl border-2 border-slate-200 bg-white p-5">
        ...
        <div className="mb-4">
          ...
          <div className={`rounded-xl border-2 px-4 py-3 text-center text-xl font-semibold ${status === "card-error" ? "border-red-300 bg-red-50 text-red-600" : "border-slate-200 bg-slate-50 text-slate-800"}`}>
            {cardId}
          </div>
        </div>
          ...
          <div className="mb-4 overflow-hidden rounded-xl bg-gradient-to-r from-slate-700 to-slate-900 p-4 text-white">
            ...
            <div className="flex items-baseline justify-between">
              <p className="text-4xl font-bold">
                {actualFee.toLocaleString()}
                <span className="text-lg">VNĐ</span>
              </p>
              <p className="text-xs text-slate-400">{customerType === "internal" ? "Tự động ghi nợ" : "Thanh toán trực tiếp"}</p>
            </div>
          </div>
        )}
        ...
        <button
          ...
          className={`mb-3 w-full rounded-xl py-4 text-lg font-bold text-white transition-colors ${
            status === "security-warning" ? "cursor-not-allowed bg-slate-400" : "bg-blue-600 hover:bg-blue-700"
          }`}
        ...
        <div className="flex gap-3">
          <button onClick={onReportLostCard} className="flex-1 rounded-xl border-2 border-slate-200 bg-white py-3 font-semibold text-slate-600 transition-colors hover:bg-slate-50">
            BÁO MẤT THẺ
          </button>
          <button onClick={onCallSupport} className="flex-1 rounded-xl border-2 border-slate-200 bg-white py-3 font-semibold text-slate-600 transition-colors hover:bg-slate-50">
            GỌI HỖ TRỢ
          </button>
        </div>
      </div>
