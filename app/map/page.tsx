...
import { Search, MapPin, Navigation, Map as MapIcon, User, ShieldAlert, ArrowLeft, Car, X, AlertOctagon } from 'lucide-react';
...
// Mock Data đã được chỉnh lại tổng số /12 để khớp với lưới 12 slot bên trong
const initialZones = [
  { id: 'A', name: 'Khu A (Gần cổng)', status: 'green', spots: '4/12', type: 'Sinh viên' },
  { id: 'B', name: 'Khu B (Trung tâm)', status: 'yellow', spots: '10/12', type: 'Sinh viên' },
  { id: 'C', name: 'Khu C (Căn tin)', status: 'red', spots: '12/12', type: 'Sinh viên' },
  { id: 'D', name: 'Khu D (Khuất)', status: 'gray', spots: '--/12', type: 'Sinh viên', note: 'Mất kết nối IoT' },
  { id: 'E', name: 'Khu Cán bộ GV', status: 'maintenance', spots: '0/12', type: 'Cán bộ', note: 'Đang bảo trì' },
  { id: 'F', name: 'Khu F (Mở rộng)', status: 'green', spots: '2/12', type: 'Sinh viên' },
];
...
export default function ParkingApp() {
  ...
  // Hàm sinh ô đỗ khớp chính xác với số lượng spot bên ngoài
  const generateSlots = (zoneId: string, status: string): Slot[] => {
    return Array.from({ length: 12 }).map((_, i) => {
      let isOccupied = false;
      // Trùng khớp với số lượng trên thẻ:
      if (status === 'green' && zoneId === 'A') isOccupied = [0, 3, 6, 9].includes(i); // 4/12
      else if (status === 'green' && zoneId === 'F') isOccupied = [1, 5].includes(i); // 2/12
      else if (status === 'yellow') isOccupied = ![2, 7].includes(i); // 10/12
      else if (status === 'red') isOccupied = true; // 12/12
  ...
  const handleZoneClick = (zone: Zone) => {
    ...
    if (zone.status === 'gray') return; // Mất kết nối thì không cho làm gì cả
    
    // XỬ LÝ LỖI: Chặn click khi bãi đã đầy
    if (zone.status === 'red') {
      setWarningInfo({
        show: true,
    ...
    setCurrentZoneSlots(generateSlots(zone.id, zone.status));
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans text-[#1E293B]">
                  ...
                  {filteredZones.map((zone) => (
                    <div 
                      ...
                      className={`
                        ...
                        ${zone.status !== 'gray' ? 'cursor-pointer hover:-translate-y-1' : ''}
                      `}
                    >
      ...
      {/* Tích hợp chung 1 Modal cho cả Cảnh báo lỗi & Hết chỗ */}
      {warningInfo.show && (
        <div className="fixed inset-0 bg-[#1E293B]/50 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-[400px] p-8 text-center shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="w-16 h-16 bg-red-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
              <AlertOctagon size={32} className="text-[#EF4444]" />
            </div>
            <h3 className="font-bold text-xl mb-2 text-[#1E293B]">{warningInfo.title}</h3>
            <p className="mb-8 text-[#64748B] text-sm leading-relaxed">{warningInfo.message}</p>
            <button 
              onClick={() => setWarningInfo({show: false, title: '', message: ''})} 
              className="w-full py-3.5 bg-[#1E293B] text-white rounded-xl font-bold hover:bg-[#0f172a] transition-colors"
            >
              Đã hiểu
            </button>
          </div>
