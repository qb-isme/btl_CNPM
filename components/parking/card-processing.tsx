...
export function CardProcessing({
  ...
  const getTitle = () => {
    if (formStatus === "parking-full") {
      return "THÔNG BÁO - BÃI ĐẦY"
    }
    return "XỬ LÝ QUẸT THẺ"
  }

  const getCustomerTypeLabel = (type: CustomerType) => {
    return type === "visitor" ? "Khách vãng lai" : "Nội bộ (Sinh viên/ Giảng viên)"
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Title */}
      <h2 className="text-2xl font-bold text-slate-800 text-center">{getTitle()}</h2>

      {/* Error/Warning Message */}
      {(isError || isWarning) && (
        <div className={`rounded-xl border px-4 py-2 text-center text-sm ${
          isWarning 
        ...
        <input
          ...
          className={`rounded-xl border-2 px-4 py-3 text-center text-xl font-semibold transition-colors ${
            isError
              ...
              : "border-slate-200 bg-slate-50 text-slate-800 focus:border-blue-400 focus:outline-none"
          }`}
      ...
      <div className="mt-2 flex gap-4">
        <button
          ...
          className="flex-1 rounded-xl bg-blue-600 px-6 py-4 font-semibold text-white transition-colors hover:bg-blue-700"
        >
          {isOffline ? (
            <span>
              Tạo phiên<br />(OFFLINE)
            </span>
          ) : (
            "Tạo phiên"
          )}
